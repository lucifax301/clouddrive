...
(function($){
    ...
    jQuery.fn.highCheckTree = function(settings){
        ...
        function getChildrenHtml(treesdata){
            ...
            for(i = 0; i < len; i++){
                node = treesdata[i];
                $.data($tree, 'a'+node.id, node); //attach node data to node id
                clen = node.children ? node.children.length : 0;
                arrowClass = 'collapsed';
                if(clen === 0){
                    arrowClass = 'nochildren';
                    checkClass = node.layoutChecked ? 'checked' : '';
                }else{
                    var checkedChildren = $.grep(node.children, function(el){
                        return el.layoutChecked;
                    });
                    checkClass = checkedChildren.length === 0 ? '' : checkedChildren.length === clen ? 'checked' : 'half_checked'; 
                }
//				function ifchildren(json){
//	               	var text = '';
//	               	if(json && json.length>0){
//	               		console.log(json)
//	               		text = '<ul>' + getChildrenHtml(json) + '</ul>';
//	               	}
//	               	return text
//	            }
               result += '<li rel="a' + node.id + '"><div class="arrow ' + arrowClass + '"></div><div class="checkbox ' + checkClass + '"></div><label title="'+node.remark+'">' +  node.name + '</label></li>';
