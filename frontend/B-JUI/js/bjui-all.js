...
+function ($) {
    ...
    var BJUI = {
        ...
        initLayout: function() {
            ...
            $('#bjui-navtab .tabsPageContent').height(hh - th - nh - tnh+7)
            ...
            $collapse.each(function() {
                ...
                console.log($this.scrollTop(),$this.hasClass('navbar-show'))
    ...
    $(function() {
        ...
        $.validator && $.validator.config({
            ...
            rules: {
                ...
                ,plusnumber:[/^\+?[0-9]+\.?[0-9]{0,2}$/,'只能输入正数,且最多两位小数']
                ,plusdigits:[/^\+?[0-9]+$/,'只能输入正整数']
                ,upletterandnumber: [/^[A-Z0-9]+$/, '只能输入大写字母和数字']
                ,tel: [/^(?:(?:0\d{2,3}[\- ]?[1-9]\d{6,7})|(?:[48]00[\- ]?[1-9]\d{6}))$/, "请填写有效的电话号码"]
                //,mobile: [/^1[3-9]\d{9}$/, '手机号格式不正确']
                ,mobile: [/^1[3|4|5|8][0-9]\d{4,8}$/, '手机号格式不正确']
                ...
                ,carno:function(element, params){
                    var carno = $(element).val(),
                        exists = false,
                        dom = $.CurrentNavtab;
                    if(params && params[0] == 'Dialog'){
                        dom = $.CurrentDialog;
                    }
                    BJUI.ajax('doajax',{
                        url: config.openBasePath + 'car/getCar?carNo=' + carno,
                        async:false,
                        okCallback: function(data) {
                            if (data.result && data.result.carId) {
                                exists = true;
                                dom.find('#carid').val(data.result.carId);
                                dom.find('#brandid').val(data.result.brandId);
                                dom.find('#areaid').val(data.result.areaId);
                            }
                        }
                    })
                    return exists?true:this.renderMsg('车牌不存在！')
                }
                ,posnum:function(element, params){
                    var posnum = $(element).val(),
                        valid = true;
                    if(params && params[0] && params[0] == posnum){
                        return
                    }
                    BJUI.ajax('doajax',{
                        url: config.openBasePath + 'pos/financePosList?posnumequal=' + posnum,
                        type: 'GET',
                        async: false,
                        okCallback: function(data) {
                    ...
                    return valid?true:this.renderMsg('Pos机号已存在！');
                }
                ,lessThan:function(element, params){
                    if(!params){return}
                    params = params[0].split(',')
                    var thisval = $(element).val(),
                        thanval = $(element).parents('form').find('[name="'+params[0]+'"]').val();
                        console.log(thisval,thanval)
                    if(!thanval){return}
                    return thisval<thanval?true:this.renderMsg('该数据应小于'+params[1]+'！');
                }
                ,greaterThan:function(element, params){
                    if(!params){return}
                    params = params[0].split(',')
                    var thisval = $(element).val(),
                        thanval = $(element).parents('form').find('[name="'+params[0]+'"]').val();
                    if(!thanval){return}
                    return thisval>thanval?true:this.renderMsg('该数据应大于'+params[1]+'！');
                }
                //检验手机号查重
                ,checkphone:function(element, params){
                    var phone = $(element).val(),
                        ...
                        type = params[0] || null
                    if(!type){
                        return this.renderMsg('未设置类型！')
                    }
                    var myreg = /^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/; 
					if(!myreg.test(phone)){ 
					    return this.renderMsg('请输入有效的手机号码！')
					} 
                    BJUI.ajax('doajax',{
                        url: window.config.openBasePath + config[type].url,
                        type: 'GET',
                        async: false,
                        data:{
                        	phone:phone
                    ...
                    return valid?true:this.renderMsg('该手机号已存在！');
                }
                //检验身份证查重
                ,checkidcard:function(element, params){
                    var phone = $(element).val(),
                        valid = true,
                        config = [
                        	{//学员
                    ...
                    var myreg = /^\d{6}(19|2\d)?\d{2}(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)?$/; 
					if(!myreg.test(phone)){ 
					    return this.renderMsg('请输入有效的身份证号码！')
					} 
                    BJUI.ajax('doajax',{
                        url: window.config.openBasePath + config[type].url,
                        type: 'GET',
                    ...
                    return valid?true:this.renderMsg('该身份证号码已存在！');
                }
            },
...
+function ($) {
    ...
    window.FRAG = {
        ...
        + '<span class="filter-ok"><button type="button" class="btn-blue ok" data-icon="check">#filter#</button><button type="button" class="btn-blue clear" data-icon="remove">#clear#</button></span>'
        + '</fieldset>'
        + '</div>'
        ,
        ...
        + '    <li><button type="button" class="btn btn-blue save" data-icon="save">#save#</button></li>'
        + '</ul>'
        ,
        ...
        alertBoxFrag: '<div id="bjui-alertMsgBox" style="width:#boxwidth#px;margin-left:#ml#px" class="bjui-alert"><div class="alertContent"><div class="#type#"><div class="alertInner"><h1><i class="fa #fa#"></i>#title#</h1><div class="msg" style="max-height:#boxheight#px;height:#boxheight#px;">#message##prompt#</div></div><div class="toolBar clearfix"><ul>#btnFragment#</ul></div></div></div></div>',
        ...
        '            <button type="button" class="okBtn btn btn-blue">#ok#</button>' +
        '        </div>' +
        '        <div class="tm">' +
        '            <ul class="hh">' +
        '                <li>0</li>' +
...
+function ($) {
    ...
    $.fn.extend({
        ...
        ajaxUrl: function(op) {
            ...
            $.ajax({
                ...
                statusCode : {
                    ...
                    405  : function(xhr, ajaxOptions, thrownError) {
                        BJUI.alertmsg('error', BJUI.regional.ajaxnosend)
                    },
        ...
        doAjax: function(op) {
            ...
            op.crossDomain = true;
            op.async = op.async == false ? false : true;
...
+function ($) {
    ...
    Sidenav.prototype.ajaxHnav = function() {
        ...
        var createA = function(data) {
            ...
            if(data.children){
                ...
                liHtml.add('><i class="fa f10 move-top-15 fa-'+ (data.icon || 'circle') +'"></i>&nbsp;'+ data.name)
            }
        ...
        options.tree ? treeCallback(options.json) : okCallback(options.json);
    }
    ...
    $(document).on('click.bjui.sidenav.data-api', '[data-toggle="sidenav"]', function(e) {
        e.preventDefault()
        var $body = $('body'), $this = $(this), href =  $this.data('name'), cache = $this.data('cache'),
...
//              BJUI.navtab('reload', 'navtab-test-1')
//          }
            ...
            var morejson = '';
            if(options.more){
                morejson = JSON.stringify(options.more);
                morejson = morejson.replace(/"/g,'\'');
            }
            var tabFrag = '<li><a href="javascript:" title="#title#" data-privilege="'+options.id+'" data-more="' + morejson +'"><span>#title#</span></a><span class="close">&times;</span></li>',
    ...
    Dialog.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            resizeBjuiRow: function($dialog) {
                ...
                //2017.3.27 弹出层打开时  权限处理
                clz.privilege_Init.start()
            }
        }
...
+function ($) {
    ...
    Bjuiajax.prototype._ajaxform = function($form, options) {
        ...
        if ($uploaders.length && WebUploader) {
            $uploaders.each(function(i, n) {
                ...
                if (uploader && uploader.options.required) {
                    if (!fileCount) {
                        requiredMsg = uploader.options.requiredMsg || '请上传文件！'
                        return false
                    }
                }
            })
        ...
        if (error4Uploader) {
            BJUI.alertmsg('info', '表单将在文件上传完成后自动提交！')
            return
        }
        ...
        var tools = {
            ...
            openPosition: function() {
                var position = BJUI.alertMsg.displayPosition,
                    mode = BJUI.alertMsg.displayMode,
                    width =  that.options.boxwidth || 460,
            ...
            open: function(type, msg, buttons) {
                ...
                if(that.options.boxheight && that.options.boxwidth){
                    $newbox =
                        $(FRAG.alertBoxFrag.replace('#type#', type)
                            ...
                            .replace('#boxwidth#', that.options.boxwidth)
                            .replace(/#boxheight#/g, that.options.boxheight)
                            .replace('#ml#', -that.options.boxwidth/2 ))
                            .hide()
                            .appendTo('body')
                }else{
                    $newbox =
...
+function ($) {
    ...
    Datagrid.DEFAULTS = {
        ...
        fieldSortable   : false,     // Click the field to sort
        filterThead     : false,     // Filter in the thead
        ...
        customEditbtns  : {         // Custom edit column buttons
            ...
            width       : 100         // The edit column width, 0 = 'auto'
        },
        ...
        toolbarItem     : 'add, |, edit, | , del, |, import, |, export',       // Displayed on the toolbar elements, Optional 'all, add, edit, cancel, save, del, import, export, exportf |'
        ...
        editType        : 'GET',   // Ajax request method of edit, Optional 'POST' | 'GET' | 'raw', (Optional 'raw' is post raw data)
        inlineEditMult  : false,     // Can inline edit multiple rows
        saveAll         : false,     // For inline edit, true = save current row, false = save all editing rows
        addLocation     : 'last',  // Add rows to datagrid location, Optional 'first' | 'last' | 'prev' | 'next'
        ...
        flowLayoutWidth : 0,        //
        dialogFilterW   : 360,      //
        hScrollbar      : true,    // Allowed horizontal scroll bar
    ...
    Datagrid.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            beforeEdit: function($trs, datas) {
                ...
                if (beforeEdit) {
                    ...
                    if (typeof beforeEdit === 'function') {
                        return beforeEdit.call(that, $trs, datas,window.table_add)
                    }
                }
            ...
            createTrs: function(datas, refreshFlag) {
                ...
                for (var i = 0, l = datas.length; i < l; i++) {
                    var trData = datas[i], modellength = model.length, tempcolspan = modellength, linenumber = options.linenumberAll ? ((paging.pageCurrent - 1) * paging.pageSize + (i + 1)) : (i + 1),
                        tds = [], n, tdHtml = BJUI.StrBuilder(), $td, name, label, _label, render_label, align,minwidth, cls, display, tree, hasTree = false, treeattr = '', tdTemplate = options.tdTemplate
                    ...
                    for (var j = 0; j < modellength; j++) {
                        ...
                        minwidth = ''//初始化minwidth 2017.4.14

                        if (typeof label === 'undefined' || label === 'null' || label === null) label = ''
                        _label = label
                        //2017 表格td默认最小宽度
                        minwidth = 'style="min-width:'+ (n.minwidth || 50) +'px"'
                        ...
                        //表格的td添加隐藏 2017.4.24
                        if (n.hide) display = ' style="display:none;"'
                        ...
                        if (that.isTemplate) {
                            ...
                            if (!tds[0] && j == (modellength - 1)) {
                                ...
                                if (that.options.showLinenumber) {
                                    ...
                                    var linenumbertd = ['<td data-title="序号" align="center" class="datagrid-linenumber-td"'+ display +'>'+ linenumber +'</td>']
                                ...
                                if (that.options.showCheckboxcol) {
                                    ...
                                    var checkboxtd = ['<td data-title="复选" align="center" class="datagrid-checkbox-td"'+ display +'><div><input type="checkbox" data-toggle="icheck" name="datagrid.checkbox" value="true"></div></td>']
            ...
            appendColumns: function() {
                ...
                that.linenumberColumn = {name:that.options.keys.gridNumber, gridNumber:true, width:30, minWidth:30, label:'序号', align:'center', menu:false, edit:false, quicksort:false}
                that.checkboxColumn   = {name:that.options.keys.gridCheckbox, gridCheckbox:true, width:30, minWidth:30, label:'复选', align:'center', menu:false, edit:false, quicksort:false}
    ...
    Datagrid.prototype.init = function() {
        ...
        if(that.options.okcallback){
            that.options.okcallback();//为表格方法添加回调事件
        }
    }
    ...
    Datagrid.prototype.setColumnModel = function() {
        ...
        if (!that.isDom) {
            ...
            if (options.showLinenumber) {
                ...
                $th = $('<th class="'+ that.classnames.td_linenumber +'"'+ _rowspan +'>序号</th>')
                $th.prependTo($trs.first())
        ...
        if ($trs.length && trLen == 1) {
            ...
            $trs.each(function(len) {
                ...
                $(this).find('> th').each(function(i) {
                    ...
                    op.label = label
                    op.th    = $th
                    if (op[options.keys.gridCheckbox]) op.label = '复选'
    ...
    Datagrid.prototype.refresh = function(filterFlag) {//表格刷新方法
    ...
    //重置数据
    Datagrid.prototype.reloaddata = function(option) {
        var that = this, options = that.options, data, $element
    ...
    Datagrid.prototype.initTop = function() {
        ...
        //获取表格的按钮权限值
        var nav_privilege_name = '',
            table_privilege = this.$element.data('tabprivilege'),
            pagePrivilege = clz.privilege_Init.Current_privilege();

        nav_privilege_name = pagePrivilege.privilege + '_' + table_privilege;
            ...
            if (options.toolbarItem || options.toolbarCustom) {
                if (options.toolbarItem) {
                    ...
                    $.each(options.toolbarItem.split(','), function(i, n) {
                        ...
                        if (n === 'add') {
                            that.$toolbar_add = $(btnHtml).attr('data-icon', 'plus').attr('data-privilege', nav_privilege_name+'_01').addClass('btn-blue').text(options.addName || BJUI.getRegional('datagrid.add'))
                                .appendTo($group)
                                .on('click', function(e) {
                                    that.add()
                                })
                        } else if (n === 'edit') {
                            that.$toolbar_edit = $(btnHtml).attr('data-icon', 'edit').attr('data-privilege', nav_privilege_name+'_02').addClass('btn-blue').text(options.editName || BJUI.getRegional('datagrid.edit'))
                                .appendTo($group)
                                .on('click', function(e) {
                            ...
                            that.$toolbar_calcel = $(btnHtml).attr('data-icon', 'undo').attr('data-privilege', nav_privilege_name+'_01').addClass('btn-blue').text(options.cancelName || BJUI.getRegional('datagrid.cancel'))
                                .appendTo($group)
                                .on('click', function(e) {
                                    that.doCancelEditRow(that.$tbody.find('> tr.'+ that.classnames.tr_edit))
                                })
                        } else if (n === 'save') {
                            that.$toolbar_save = $(btnHtml).attr('data-icon', 'save').attr('data-privilege', nav_privilege_name+'_01').addClass('btn-blue').text(options.saveName || BJUI.getRegional('datagrid.save'))
                                .appendTo($group)
                                .on('click', function(e) {
                                    that.doSaveEditRow()
                                })
                        } else if (n === 'del') {
                            that.$toolbar_del = $(btnHtml).attr('data-icon', 'times').attr('data-privilege', nav_privilege_name+'_03').addClass('btn-red').text(options.delName || BJUI.getRegional('datagrid.del'))
                                .appendTo($group)
                                .on('click', function(e) {
                                    var $selectTrs = that.$tbody.find('> tr.'+ that.classnames.tr_selected)
                            ...
                            that.$toolbar_refresh = $(btnHtml).attr('data-icon', 'refresh').attr('data-privilege', nav_privilege_name+'_04').addClass('btn-blue').text(options.refreshName || BJUI.getRegional('datagrid.refresh'))
                                .appendTo($group)
                                .on('click', function(e) {
                                    that.refresh()
                                })
                        } else if (n === 'import') {
                            that.$toolbar_add = $(btnHtml).attr('data-icon', 'sign-in').attr('data-privilege', nav_privilege_name+'_05').addClass('btn-blue').text(options.importName || BJUI.getRegional('datagrid.import'))
                                .appendTo($group)
                                .on('click', function(e) {
                            ...
                            that.$toolbar_add = $(btnHtml).attr('data-icon', 'sign-out').attr('data-privilege', nav_privilege_name+'_06').addClass('btn-blue').text(options.exportName || BJUI.getRegional('datagrid.export'))
                                .appendTo($group)
                                .on('click', function(e) {
    ...
    Datagrid.prototype.initThead = function() {
        ...
        //对表格hide处理  col部分 2017.4.25
        $.each(that.columnModel, function(i, n) {
            if(n.hide){
                that.$colgroupH.find('> col').eq(i).css('display', 'none')
                that.$colgroupB.find('> col').eq(i).css('display', 'none')
            }
        })
    }
    ...
    Datagrid.prototype.add = function(addLocation, addData, noEdit) {
        //使用添加打开窗口)
        window.table_add = true;
        if(this.options.add_url){
            this.options.editUrl = this.options.add_url;
        }
        ...
        var addTr = function() {
            ...
            if (that.isTemplate) {
                ...
                if (that.options.showLinenumber) {
                    $tr.append('<td data-title="序号" align="center" class="datagrid-linenumber-td>0</td>')
                    tempcolspan --
                }
                if (that.options.showCheckboxcol) {
                    $tr.append('<td data-title="复选" align="center" class="datagrid-checkbox-td"><div><input type="checkbox" data-toggle="icheck" name="datagrid.checkbox" value="true"></div></td>')
                    tempcolspan --
                }
                ...
                $.each(that.columnModel, function(i, n) {
                    ...
                    if (n.hidden || n.hide) $td.css('display', 'none')
    ...
    Datagrid.prototype.externalEdit = function(row, editOpts, data) {
        if(this.options.edit_url && !window.table_add){
            this.options.editUrl = this.options.edit_url;
        }
        var that = this, options = that.options, editOptions = {}, $tr, data_index, editUrl, type, types = ['dialog', 'navtab', 'div']
        //阻止编辑多条
        var edit_che_data = that.$element.data('selectedDatas');
        if(edit_che_data && edit_che_data.length > 1 && !window.table_add){
        ...
        var doDel = function() {
            ...
            if (options.delUrl) {
                ...
                var type = options.delType, opts = {
                    url:options.delUrl,
                    data : {ids : postData.join(',')},
                    //data:(options.delPK ? [{ name:options.delPK, value:postData.join(',') }] : JSON.stringify(postData)),
                    type:'POST',
                    okCallback:callback || function(json) {console.log(json); delEnd() }}
        ...
        if (!len) {
            ...
            if (that.options.saveLocal) {
                ...
                if (that.tools.beforeSave($trs, datas)) {
                    //多行提交接口 POST
                    var type = 'POST', opts = {url:options.editUrl, data:JSON.stringify(postData), type:'POST', okCallback:callback}
            ...
            $tr.isValid(function(v) {
                if (v) {
                    ...
                    if (that.tools.beforeSave($tr, datas)) {
                        postData.push(tempData)
                        var type = options.editType, opts = {url:options.editUrl, data:postData[0], type:'POST', okCallback:callback}
    ...
    Datagrid.prototype.cancelEdit = function($trs) {
        ...
        $trs.each(function() {
            ...
            if ($tr.hasClass(that.classnames.tr_add)) {
                data_index = that.tools.getNoChildDataIndex(data_index)
                if(data_index == -1){return}//2017.3.4 reload改了异常处理
                if (!that.isDom) {
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        if ($.fn.selectpicker) {
            ...
            var bjui_select_linkage = function($obj, $next) {
                ...
                var refurl  = $obj.data('refurl')?(window.IP + $obj.data('refurl')):'';
                var refdata = $obj.data('refdata') || null;
                var reffilter  = $obj.data('reffilter');
                var _setEmpty = function($select) {
                    //var $_nextselect = $(($select.data('nextselect'))
                    if($.CurrentDialog){
                        var thisdom = $.CurrentDialog
                    }else{
                        var thisdom = $.CurrentNavtab
                    }
                    var $_nextselect = thisdom.find($select.data('nextselect'))
                ...
                if (($next && $next.length) && (refurl || refdata)) {
                    ...
                    if(refurl){
                        ...
                        refdata = refdata.replace('{value}', encodeURIComponent(val))
                        refdata = refdata.split(',')
                        var getjson = {name:refdata[0],belong:refdata[1],belongkeys:refdata[2],filter:reffilter}
                        var json =  clz.getdictionary(getjson)
                        echooption(json)
                    }
                    function echooption(json){
                    	if(json.list){
                    		json = json.list
                    	}
                        var qxz = {}
                        qxz[keys.value || 'value'] = ''
                        qxz[keys.label || 'label'] = '请选择'
                        json.unshift(qxz);
                        var html = '', selected = ''
                        $.each(json, function(i) {
                            ...
                            if (typeof nextVal !== 'undefined' || nextVal === 0) selected = value == nextVal? ' selected' : ''
            ...
            //带ajax的下拉框
            var bjui_select_init = function($obj) {
                var thisdata = $obj.data('optiontype') || 'val,name';//val和name数据类型
                if(thisdata){
                    thisdata = thisdata.split(',');
                }
                var dataval = $obj.data('val');//获取当前项目选中值
                console.log(dataval,$obj)
                if($obj.data('dataurl')){
                    var url = window.IP + $obj.data('dataurl');
                    $.ajax({
                        type     : 'get',
                        dataType : 'json',
                        url      : url,
                ...
                }else if($obj.data('datajson')){
                    var json = clz.getdictionary({name:$obj.data('datajson'),filter:$obj.data('filter')})
                    pushoption(json)
                }
                function pushoption(json){
                    ...
                    $.each(json, function(i) {
                        ...
                        if (json[i]) {
                            if(!thisdata){
                                ...
                                value = json[i][thisdata[0]]
                                label = json[i][thisdata[1]]
                            }
                        }
                        var nextVal = dataval;
                        if (nextVal || nextVal === 0)selected = value == nextVal ? 'selected' : ''
                        html += '<option value="'+ value +'"'+ selected +'>' + label + '</option>'
                    })
                    $obj.html(html).selectpicker('refresh');
                    $obj.trigger(BJUI.eventType.initUI)//重新渲染BJUI
                }
            }
            $selectpicker.each(function() {
                ...
                if($element.data('dataurl') || $element.data('datajson')){bjui_select_init($element)}
            ...
            $selectpicker.change(function() {
                ...
                var $nextselect = thisdom.find($element.data('nextselect'))
                ...
                if (options) {
                    ...
                    if (typeof options === 'object') {
                        ...
                        var addFile = function(file, isuploaded, _index) {
                            ...
                            var uploadedAttr = isuploaded && upunit === '个文件' ? ' style="cursor:pointer;" data-toggle="dialog" data-options="{id:\'bjui-dialog-view-upload-image\', image:\''+ encodeURIComponent(file.src) +'\', width:800, height:500, mask:true, title:\'查看已上传文件\'}"' : '',
                        ...
                        var updateStatus = function() {
                            ...
                            if (state === 'ready') {
                                ...
                                //modify by devil 20170405 去掉后面的数字
                                text = '共'+ fileCount + upunit +'（' + WebUploader.formatSize(fileSize) +'），已上传'
                        ...
                        uploader.on('uploadSuccess', function(file, response) {
                            ...
                            if(options.type == "excel"){
                                if (response[BJUI.keys.statusCode] != BJUI.statusCode.ok) {
                                    if(response.result.download != undefined && response.result.download==1){
                                        ...
                                        if(response.result.error==0){
                                            ...
                                            html += '<p>共错误 <span class="red">' + response.result.error + '</span> 条数据,<a class="" target = "_blank=" href="' + window.config.openBasePath + 'excel/download/' + response.result.filename + '">下载错误数据</a></p>'
                                        }
                                        BJUI.dialog('close', 'dialog-upload-excel');
                                        if ($.CurrentNavtab.find('#datagrid-list')) {
